...
float3 TechnicolorPass(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float2 negative_mul_r = tcol.rg * (1.0 / (NegativeAmount.r * Power));
	float2 negative_mul_g = tcol.rg * (1.0 / (NegativeAmount.g * Power));
	float2 negative_mul_b = tcol.rb * (1.0 / (NegativeAmount.b * Power));
	...
	return lerp(tcol, output_r * output_g * output_b, Amount);
}
