...
float3 FilmGrainPass(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	//Get settings
	float stn = FilmGrain_SNR != 0 ? pow(inv_luma, (float)FilmGrain_SNR) : 1.0; // Signal to noise feature - Brighter pixels get less noise.
	float variance = (FilmGrain_variance*FilmGrain_variance) * stn;
