...
float4 PS_CRTGeom(float4 vpos : SV_Position, float2 uv : TexCoord) : SV_Target
{
	...
	if (CURVATURE > 0.5){
		...
		xy =  (bkwtrans(cd, sinangle, cosangle)/float2(overscan_x / 100.0, overscan_y / 100.0)/aspect+float2(0.5,0.5)) * texture_size / texture_size;
	} else {
		xy = uv;
	}

	float2 cd2 = xy;
	cd2 *= texture_size / texture_size;
