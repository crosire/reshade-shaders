...
float4 VignettePass(float4 vpos : SV_Position, float2 tex : TexCoord) : SV_Target
{
	...
	if (VignetteType == 0)
	{
		...
		//Apply the vignette
		color.rgb *= (1.0 + pow(distance, VignetteSlope * 0.5) * VignetteAmount); //pow - multiply
	}

	if (VignetteType == 1) // New round (-x*x+x) + (-y*y+y) method.
	{
		tex = -tex * tex + tex;
		color.rgb = saturate(((ReShade::PixelSize.y / ReShade::PixelSize.x)*(ReShade::PixelSize.y / ReShade::PixelSize.x) * VignetteRatio * tex.x + tex.y) * 4.0) * color.rgb;
	}

	if (VignetteType == 2) // New (-x*x+x) * (-y*y+y) TV style method.
	{
		tex = -tex * tex + tex;
	...
	if (VignetteType == 5) //MAD version of 2
	{
		tex = abs(tex - 0.5);
		//tex = abs(0.5 - tex); //same result
	...
	if (VignetteType == 6) // New round (-x*x+x) * (-y*y+y) method.
	{
