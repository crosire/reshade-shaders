...
uniform float4 AddColor < __UNIFORM_COLOR_FLOAT4
	ui_label = "Add Color";
	ui_tooltip = "Used to adjust image color down, white does nothing";
	ui_category = "Adjustments";
...
uniform int Color < __UNIFORM_RADIO_INT1
	...
	ui_items = "Super Blue Ultimatte(tm)\0Green Ultimatte(tm)\0Custom\0Alpha Layer\0";
...
float4 LumakeyPS(float4 vois : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	switch(Color)
	{
		...
		case 3:{ Screen = float3(1.0,1.0,1.0);      break; } // Alpha Layer - not yet implemented
	}
	...
	float keyAmount = clo * chi;

	if (invertAlphaChannel)
	{
		keyAmount = 1.0 - keyAmount;
	}	
	rgba *= AddColor;
	keyAmount = clamp(keyAmount,0.0,1.0);

	if (keyAmount < 1.0) {
		rgba = float4(Screen,smoothstep(1.0, 0.0, keyAmount));
	} 
