...
uniform float3 BlendColor < ui_tooltip="Specifies the blend color to blend with the greyscale. in (Red, Green, Blue). Use dark colors to darken further away objects" > = float3(0.0,0.0,0.0);
...
{
	const float scenedepth = tex2D(LinearizedDepth, texcoord).r;
	const float depthDiffCoC = CalculateDepthDiffCoC(scenedepth);	
	const float4 colFragment = tex2D(BackBuffer, texcoord);
...
technique Otis_Emphasize <bool enabled = false; int toggle = EMZ_ToggleKey; >
{
